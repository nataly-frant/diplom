...
from app.models.cache_production_plan import CacheProductionPlan


def get_or_update(db: Session, fetch_callback):
    cached = db.query(CacheProductionPlan).first()
    ...
    if cached:
        ...
        cached = CacheProductionPlan(data_json=new_data)
        db.add(cached)
    db.commit()
    db.refresh(cached)
    return cached
